...
class ExpRunner(object):
    ...
    def do_model_exp(self, experiment_dict,  experiment_params_module, logger:object)->object:
        ...
        :retrun: dictionary of performance results
        """
        logger.info('in sequential do_model_exp')
        ...
        for i in range(last_iter, 11):
            ...
            logger.info("starting sequential iteration %d for %s", i, model_base_name)
            res = self.do_5_fold_cv(experiment_params_module, logger)
...
if __name__ == "__main__":
    ...
    logger.debug("Starting experiment %s in sequential mode...", exp_name)
