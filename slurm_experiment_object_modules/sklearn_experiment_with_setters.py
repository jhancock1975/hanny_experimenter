...
class SklearnExperimentContainer(object):
    ...
    def __init__(self, name:str, experiment: object)->None:
        ...
        if 'in progress file' in experiment:
            self._in_progress_file_name = experiment['in progress file']
        ...
        if 'scaler' in experiment:
            self._scaler = experiment['scaler']
    ...
    def scaler(self):
        """
        returns features  scaler
        so far looks like min/max scaler is all we need
        """
        return self._scaler
   
    @scaler.setter
    def scaler(self, features):
        self._scaler = scaler
    ...
    def data_prep(self, df: pd.DataFrame, experiment_params)->pd.DataFrame:
        """
        check if there is a data_prep function defined for the
    ...
    @property
    def in_progress_file_name(self):
        """
        :return: name of file to save experiment results
        """
        return self._in_progress_file_name

    @in_progress_file_name.setter
    def in_progress_file(self, in_progress_file_name):
        self._in_progress_file_name = in_progress_file_name
